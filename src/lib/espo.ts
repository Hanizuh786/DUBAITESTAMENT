...
export async function createEspoRecord(payload: Record<string, unknown>) {
  ...
  if (!baseUrl || !apiKey || !entity) throw new Error("EspoCRM environment configuration is incomplete.");
  const response = await fetch(`${baseUrl}/api/v1/${encodeURIComponent(entity)}`, { method: "POST", headers: { "Content-Type": "application/json", "X-Api-Key": apiKey }, body: JSON.stringify(payload), cache: "no-store" });
